...
def expand_atoms_to_use(mol, atoms_to_use, groups=[], symbol_replacements=[], super_general=False):
        ...
        if not super_general:
            ...
            for neighbor in atom.GetNeighbors():
                if neighbor.GetIdx() not in new_atoms_to_use:
                    ...
                    symbol_replacements.append((neighbor.GetIdx(), convert_atom_to_wildcard(neighbor, super_general=super_general)))
    ...
    for mol in mols:
        ...
        for k in range(radius):
            atoms_to_use, symbol_replacements = expand_atoms_to_use(mol, atoms_to_use, 
                groups=groups, symbol_replacements=symbol_replacements, super_general=super_general)
            ...
            if expansion:
                for atom in mol.GetAtoms():
                    ...
                    if label in expansion and label not in changed_atom_tags:
                        ...
                        symbol_replacements.append((atom.GetIdx(), convert_atom_to_wildcard(atom, super_general=super_general)))
...
def extract_from_reaction(reaction, super_general=False):
    ...
    try:
        ...
        reactant_fragments, intra_only, dimer_only = get_fragments_for_changed_atoms(reactants, changed_atom_tags, 
            radius = 1, expansion = [], category = 'reactants', super_general = super_general)
